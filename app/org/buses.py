...
def toggle_bus(id):
    ...
    try:
        ...
        db.commit()
        flash("Bus status updated", "success")
        return redirect("/org/buses")
    finally:
        cursor.close()
        db.close()
...
def fuel_price_manage():
    ...
    cursor.execute("""
        SELECT id, fuel_type, price_per_unit, effective_from
        FROM fuel_price
        WHERE org_id=%s
...
@org_bp.route("/fuel-price/<int:id>/delete")
@login_required
def delete_fuel_price(id):
    ...
    try:
        ...
        db.commit()
        flash("Fuel price deleted successfully", "success")
