...
from app.org.daily_report import update_daily_report
...
def end_trip():
    ...
    try:
        ...
        if trip:
            ...
            db.commit()
            # 🔥 ADD THIS LINE
            update_daily_report(trip["trip_id"])
...
def student_attendance():
    ...
    try:
        ...
        # 🔥 REAL-TIME UPDATE AFTER PICKUP/DROP
        cursor.execute("""
            SELECT bt.id
            FROM bus_trip bt
            WHERE bt.assignment_id = (
                SELECT id FROM driver_assignment
                WHERE driver_id=%s AND assignment_date=CURDATE()
                LIMIT 1
            )
            AND bt.status='STARTED'
